...
def obtener_alumnos_por_periodo(request):
    ...
    alumnos = Alumno.objects.filter(matricula__seccion__periodo_id=periodo_id, eliminado=False).order_by('usuario__apellidoPaterno').distinct()
...
def obtener_alumnos_por_sede(request):
    ...
    # Filtrar solo los alumnos cuyo usuario no sea None
    alumnos = Alumno.objects.filter(sede_id=sede_id, eliminado=False, usuario__isnull=False).order_by('usuario__apellidoPaterno') 
...
def cargar_calificaciones(request):
    ...
    try:
        ...
        secciones_docente = Seccion.objects.filter(docente__usuario=usuario_docente)
        ...
        try:
            ...
            for seccion in secciones:
                ...
                if num_alumnos_matriculados > 0:
                    maestrias_set.add((seccion.maestria.id, seccion.maestria.codigo, seccion.maestria.nombre))
                    cursos_set.add((seccion.curso.id, seccion.curso.codigo, seccion.curso.nombre))

            # Convertir los conjuntos a listas
            maestrias = [{'id': id, 'codigo': codigo, 'nombre': nombre} for id, codigo, nombre in maestrias_set]
            cursos = [{'id': id, 'codigo': codigo,'nombre': nombre} for id, codigo, nombre in cursos_set]
...
def registro_pagos(request):
    ...
    if sede_id or alumno_id:
        # Aplicar los filtros solo si se proporciona al menos uno de los parámetros
        detalleDePago = ReporteEcoConceptoPago.objects.filter(eliminado=False)
...
def reporte_calificaciones(request):
    sedes = Sede.objects.filter(eliminado=False)
    alumnos = Alumno.objects.filter(eliminado=False)
    ...
    try:
        alumno_actual = Alumno.objects.get(usuario=usuario_actual,eliminado=False)
    except Alumno.DoesNotExist:
        pass
    
    maestria_alumno = [alumno.maestria.nombre for alumno in alumnos]
    ...
    return render(request, 'reporte_calificaciones.html', {
        'detalleAcademico': detalleAcademico,
        'maestria_alumno': maestria_alumno,
        'sedes': sedes,
        'alumnos': alumnos,
        'resultados_disponibles': resultados_disponibles,
        'alumno_actual': alumno_actual,
...
def reporteEconomico(request):
    try:
        alumno_login = Alumno.objects.get(usuario=request.user, eliminado=False)                
    except Alumno.DoesNotExist:
        alumno_login = None
    
    reporteEcon = ReporteEconomico.objects.filter(alumno__usuario=request.user, eliminado=False)
    if reporteEcon:
        detalleRepoEco = ReporteEcoConceptoPago.objects.filter(reporteEconomico=reporteEcon.first(), eliminado=False)
    else:
        detalleRepoEco = []
...
def reporteAcademico(request):
    ...
    reporteAcad = DetalleMatricula.objects.filter(matricula__alumno__usuario=request.user, eliminado=False).order_by('seccion__periodo__codigo')
...
def reporteMatricula(request):
    try:
        matriculas = Matricula.objects.filter(alumno__usuario=request.user, eliminado=False)
...
@login_required
def signout(request, redirect_to='home'):
    logout(request)
...
def generar_pdf(request):
    ...
    detalleAcademico = DetalleMatricula.objects.filter(matricula__alumno__usuario=request.user, eliminado=False).order_by('seccion__periodo__codigo')
...
def es_docente(user):
    # Verifica si el usuario está asociado a algún objeto Docente
    return user.is_authenticated and Docente.objects.filter(usuario=user).exists()
...
def editar_notas(request, seccion_id):    
    ...
    try:
        seccion = Seccion.objects.get(pk=seccion_id, eliminado=False)
    except Seccion.DoesNotExist:
        return HttpResponseNotFound("La sección solicitada no existe.")

    # Verificar si el usuario (docente) está asociado a la sección
    if seccion.docente.usuario != request.user:
        return HttpResponseForbidden("No tienes permiso para editar las notas de esta sección.")
    #------------------------------------------------------------------------------------------
        
    seccion = Seccion.objects.get(pk=seccion_id,eliminado=False)
    alumnos = seccion.matricula_set.all().order_by('alumno__usuario__apellidoPaterno', 'alumno__usuario__primerNombre')
    definiciones = DefinicionCalificacion.objects.filter(seccion=seccion,eliminado=False)
     # Define success con un valor predeterminado
    success = False
    if request.method == 'POST':
            ...
            for definicion in definiciones:
                ...
                if nota_str:
                    ...
                    calificacion, created = Calificacion.objects.get_or_create(
                        detalle_matricula=DetalleMatricula.objects.get(matricula=alumno, seccion=seccion,eliminado=False),
                    ...
                    if calificacion.pk:
                        success = True  # Cambia el indicador a True si la calificación se guardó correctamente
        
    return render(request, 'editar_nota.html', {
        'seccion': seccion,
        'alumnos': alumnos,
        'definiciones': definiciones,
        'success': success  # Pasamos el indicador de éxito al template
    })
...
def generar_pdf_administrativo(request):
    ...
    alumnos = Alumno.objects.filter(id=alumno_id)
    ...
    especialidad = maestria_alumno[0]
...
def generar_pdf_pagos(request):
    ...
    width, height = A4
    table.wrapOn(c, width, height)
    table.drawOn(c, 30, 620)  # Ajusta el valor '600' según sea necesario
    c.showPage()  # save page
